...
6. Почти все шаблоны словаря намеренно включают начало слова: необходимо исправлять не только ударения в словах, но и массу других ошибок. Для данного случая обработка шаблонами только ударных буквосочетаний не является приемлемым. Ни одна словоформа не обрабатывается двумя шаблонами (кроме исключений в виде regex и нескольких шаблонов с кавычками), проверка на двойные срабатывания производится для каждой версии и предполагается, что ошибки этого типа исключены.
...
regex"(?i)(?<!(\b([гм]|зел[её]но|из|как|кое|красно|пришло|светло|серо|темно|яйце)|[0-9IVXL ]))[-'](?!то\b|таки\b|временн|образн|[ст]воему\b|моему\b|\u0320)"=" "

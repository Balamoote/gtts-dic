...
5. Словарь намеренно сделан таким образом, чтобы охватить максимальное количество словоформ, вне зависимости от того, произносится слово движком правильно или нет. Вызвано это тем, что на практике невозможно отловить все ошибки в обозримое время, а офф-лайновый gtts в части корректности произношения развивается медленно. Достаточно сказать, что корпус словаря "правильно" (т.е. с ударение так же как в словаре) произносится только на менее, чем 20%. В результате словарь получился большой, что положительно сказалось на качестве поизношения, но несколько пострадала производительность.
6. Почти все шаблоны словаря намеренно включают начало слова. Предполагается, что ни одна словоформа не обрабатывается двумя шаблонами, хотя отдельные ошибки могут еще встречаться. Исключения: см в блоке regex и в шаблонах без пробела после начальной кавычки.
...
1. Словарь будет полностью ё-фицирован со временем. Пока только частично.

Слова с дефисом.
1. дефис удаляется и части слова с дефисом обрабатываются отдельно. Исключение сделано для некоторых элементов и сочетаний.
2. особые словосочетания можно "исправить" записав через пробел и взв в кавычки: папье-маше --> " папье маше "=" па'пье-маше' "
3. регекс, отвечающий за это поведение: regex"(?<!\b[Гг]|\b[Ии]з|\b[Кк]ак|\b[Кк]ое|\b[Кк]расно|\b[Мм]\d)-(?!то|либо|нибудь|таки|образн\B|[ст]воему|моему)\b"=" " : т.е. элементы "г", "из", "как", "кое", "красно", "м" до дефиса и элементы "то", "либо", "нибудь", "таки", "образн", "своему", "твоему", "моему" после дефиса в слове. Некоторые из них, скорее всего, будет в последствии удалены.
4. оставшиеся "нерабочие" шаблоны из словаря в процессе удаления.
